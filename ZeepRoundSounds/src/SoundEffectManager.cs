...
using FMODUnity;
using UnityEngine;
using ZeepRoundSounds.src;
using ZeepRoundSounds.src.Patches;

public class SER_SoundEffectManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ModConfig.sFX_volume.SettingChanged += OnVolumeChanged;
    ...
    {
        globalVolume = obj;
        SetGroupVolume((float)(((float)ModConfig.sFX_volume.Value / 100) * globalVolume * 0.01 * baseVolume));
    }
    ...
    {
        FMOD.RESULT result = RuntimeManager.CoreSystem.createChannelGroup("StartEndRoundSounds", out customChannelGroup);
